...
from hunter.localuseragent import ua
from hunter.instruments import TrioProgress
...
def get_functions(modules,args=None):
    """Transform the modules objects to functions, skipping stubs"""
    websites = []
    ...
    for module in modules:
        if len(module.split(".")) > 3 :
            ...
            site = module.split(".")[-1]
            # Skip stub modules
            if hasattr(modu, 'is_stub') and getattr(modu, 'is_stub'):
                continue
...
def check_update():
    """Check and update hunter if not the last version"""
    return  # Skip update check for now
    check_version = httpx.get("https://pypi.org/pypi/hunter/json")
    if check_version.json()["info"]["version"] != __version__:
        ...
        p_status = p.wait()
        print("Hunter has just been updated, you can restart it.")
        exit()

def credit():
    """Print Credit"""
    print('Github : https://github.com/whoamikiddie/hunter')
...
def print_result(data,args,email,start_time,websites):
    ...
    used, not_used, rate_limit, error = 0, 0, 0, 0
    for results in data:
    ...
    print("\n" + description)
    print(f"Summary: {used} used, {not_used} not used, {rate_limit} rate limited, {error} errors.")
...
def export_csv(data,args,email):
    ...
    if args.csvoutput == True:
        ...
        name_file="hunter_"+str(round(timestamp))+"_"+email+"_results.csv"
...
async def launch_module(module,email, client, out):
    ...
    try:
        ...
        out.append({"name": name,"domain":data[name],
                    ...
                    "others": None})
async def maincore():
    parser= ArgumentParser(description=f"hunter v{__version__}")
    ...
    if not is_email(email):
        exit("[-] Please enter a target email ! \nExample : hunter email@example.com")

    # Import Modules
    print("Debug: Importing modules...")
    modules = import_submodules("hunter.modules")
    print(f"Debug: Found {len(modules)} modules")
    websites = get_functions(modules,args)
    print(f"Debug: Found {len(websites)} website functions")
    # Get timeout
    ...
    print("Debug: Starting module execution...")
    async with trio.open_nursery() as nursery:
        for website in websites:
            nursery.start_soon(launch_module, website, email, client, out)
    trio.lowlevel.remove_instrument(instrument)
    print(f"Debug: Module execution complete. Got {len(out)} results")
