...
async def medium(email, client, out):
    ...
    try:
        ...
        check = await client.post(
            "https://medium.com/_/api/users/email",
            json={
                "email": email,
        ...
        if response_data.get("error", {}).get("code") == "email_taken":
            # Try to get additional account info
            try:
                # Try to get public profile info
                profile_check = await client.get(
                    f"https://medium.com/@{email.split('@')[0]}",
                    headers=headers
                )
                
                if profile_check.status_code == 200:
                    # Extract profile info from response
                    profile_match = re.search(r'<title>([^<]+) – Medium</title>', profile_check.text)
                    if profile_match:
                        profile_name = profile_match.group(1)
                        
                        # Try to get profile picture
                        pic_match = re.search(r'<meta property="og:image" content="([^"]+)"', profile_check.text)
                        profile_pic = pic_match.group(1) if pic_match else None
                        
                        # Try to get bio
                        bio_match = re.search(r'<meta name="description" content="([^"]+)"', profile_check.text)
                        bio = bio_match.group(1) if bio_match else None
                        
                        others = {
                            "name": profile_name,
                            "profile_url": f"https://medium.com/@{email.split('@')[0]}",
